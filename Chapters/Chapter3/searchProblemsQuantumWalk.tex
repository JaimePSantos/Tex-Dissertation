...
\end{equation}
where M is the set of marked vertices and $\mmathcal{O}$ is an analogue to Grover's oracle. For one marked vertex, this oracle can be written as 
%TODO:\textcolor{red}{você pode até dizer que $M=\{0\}$}
\begin{equation}
	\mmathcal{O} = I - 2 \ket{0}\bra{0}
\end{equation}
...
\begin{equation}
	\mathcal{O'} =\mmathcal{O}\otimes I = (I_N - 2\ket{0}\bra{0})\otimes I_N = I_{N^2} - 2 \sum_v \ket{0}\ket{v}\bra{0}\bra{v} ,
\end{equation}
...
\begin{equation}
	U' = S(I \otimes G)\mathcal{O'} = S(I \otimes G)\mmathcal{O} \otimes I = S (\mmathcal{O} \otimes G),\label{eq:modifiedEvoCoined}
\end{equation}
